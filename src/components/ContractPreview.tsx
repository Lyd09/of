...
const numberToWords = (num: number) => {
    ...
    return words[num - 1] || `${num}ª`;
};

const Clause: React.FC<{ title: string; number: number; children: React.ReactNode }> = ({ title, number, children }) => (
  <div className="mb-4">
    <p className="font-bold">
      {boldenContractTerms(`Cláusula ${numberToWords(number)} – ${title.toUpperCase()}`, termsToBold)}
    </p>
...
export function ContractPreview({ data }: ContractPreviewProps) {
  ...
  return (
    <div className="bg-white text-black p-12 shadow-lg" style={{ fontFamily: 'Arial, Helvetica, sans-serif' }}>
      <h1 className="text-center font-bold text-lg mb-8">{contractTitle || 'TÍTULO DO CONTRATO'}</h1>

      <div className="mb-6 space-y-4">
        <p className="font-bold text-justify">Pelo presente instrumento particular, as partes qualificadas abaixo, de um lado:</p>
        <div>
